...
//!
//! Each pair (p1, p2) is two offsets inside a 31×31 patch centered on a
//! keypoint. After rotating by the keypoint orientation θ, the descriptor
//! bit is I(p1) < I(p2) on the smoothed patch.
//!
//! Source: Rublee et al., "ORB: An efficient alternative to SIFT or SURF"
//! (ICCV 2011), as adopted verbatim by ORB-SLAM3. The pattern was learned
//! on ~300k keypoints to maximize per-bit variance (~0.5) and minimize
...
/// Typed view over [`RAW`]. Built at compile time; zero runtime cost.
pub static BRIEF_PATTERN: [BriefPair; BRIEF_PATTERN_LEN] = {
    ...
    let mut i = 0;
    while i < BRIEF_PATTERN_LEN {
        out[i] = BriefPair {
            p1: (RAW[4 * i], RAW[4 * i + 1]),
            p2: (RAW[4 * i + 2], RAW[4 * i + 3]),
        };
        i += 1;
    }
    out
